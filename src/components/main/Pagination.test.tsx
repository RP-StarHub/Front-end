...
describe("Pagination 컴포넌트", () => {
  const setup = (props = {}) => {
    ...
    return {
      getNextButton: () => screen.getByRole('button', { name: /다음/ }),
      getPrevButton: () => screen.getByRole('button', { name: /이전/ }),
      getPageButton: (num: number) => screen.getByRole('button', { name: num.toString() }),
  ...
  describe("사용자 동작 테스트", () => {
    ...
    it("연속으로 다음 버튼을 클릭하면 페이지가 순차적으로 증가해야 한다", () => {
      // Arrange
      let currentPage = 1;
      const { getNextButton, mockOnPageChange, rerender, getPageButton } = setup({
        currentPage,
        totalPages: 5
      });
    ...
    it("연속으로 이전 버튼을 클릭하면 페이지가 순차적으로 감소해야 한다", () => {
      // Arrange
      let currentPage = 3;
      const { getPrevButton, mockOnPageChange, rerender, getPageButton } = setup({
        currentPage,
        totalPages: 5
      });
    ...
    it("마지막 페이지에서는 다음 버튼이 비활성화되어야 한다", () => {
      // Arrange
      const { getNextButton } = setup({ currentPage: 10, totalPages: 10 });
  ...
  describe("에러 케이스 테스트", () => {
    it("totalPages가 0이거나 음수일 때 모든 페이지 버튼이 보이지 않아야 한다", () => {
      // Arrange
      const { getPrevButton } = setup({ totalPages: 0 });
    ...
    it("currentPage가 totalPages보다 크면 onPageChange가 호출되지 않아야 한다", () => {
      // Arrange
      const { getNextButton, mockOnPageChange } = setup({
        currentPage: 10,
        totalPages: 10
      });
  ...
  describe("접근성 테스트", () => {
    it("이전/다음 버튼에는 적절한 aria-label이 있어야 한다", () => {
      // Arrange
      const { getPrevButton, getNextButton } = setup();

      // Assert
      expect(getPrevButton()).toHaveAttribute("aria-label", "이전 페이지");
      expect(getNextButton()).toHaveAttribute("aria-label", "다음 페이지");
    });
